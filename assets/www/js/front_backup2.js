								...
								function(tx, results) {
									...
									for ( var i = 0; i < numItems; i++) {
										markup += '<li id="item_'
												...
												+ '"><a href="#editpage">'
												+ '<img src="'
												+ results.rows.item(i).thumbnail_url
												+ '"/>'
												+ '<h3>'
												+ results.rows.item(i).name
												+ '</h3>'
												+ '<p>Added on date '
												+ date2shortString(new Date(
														results.rows.item(i).added_date))
												+ '</p>'
												+ expireHtml(results.rows
														.item(i).expire_date)
												+ '</a>'
												+ '<a class="deleteButton" href="#" onclick="deleteItem('
												+ results.rows.item(i).item_id
												+ ', \''
												+ results.rows.item(i).name
												+ '\')">delete</a>' + "</li>";
...
function setEditItem(name, thumbnail_url, length, unit) {
	$('#name').val(name);
